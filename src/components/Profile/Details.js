...
const Details = ({ userDetails, user, user1, children, child, notProfile }) => {
                ...
                <form className="space-y-5  flex flex-col justify-center items-center">
                    <div className="w-full space-y-2">
                        ...
                        <div className="sm:grid sm:grid-cols-2 sm:gap-2">
                            <Input update={(e) => { setUserData({ ...userData, fname: e.target.value }) }} value={
                                (notProfile && user) ? userDetails.fname : userData.fname
                            ...
                            <Input update={(e) => { setUserData({ ...userData, email: e.target.value }) }} disabled={(userData.email === user?.email || userData.email === user1?.email) ? true : false} value={userData.email} className="w-full mb-5 sm:mb-auto" type={"email"} required placeholder={"E-mail"} />
                        </div>
                    </div>
                    <div className="w-full space-y-2">
