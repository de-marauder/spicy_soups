...
const Sidebar = (props) => {
    ...
    return (
        !props.sidebar ? <aside className={`z-50 flex sm:hidden pl-3 py-10 justify-around text-white text-3xl flex-col absolute w-3/4 h-full -right-3/4 transition-all bg-black ${props.style}`}>
            <MdRestaurantMenu 
                className="absolute top-8 right-10 hover:text-red-700"
                onClick={props.doStuff} />
            <div className="text-3xl mt-9 text-green-00 font-black ml-4">Spicy soups</div>
